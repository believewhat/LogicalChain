...
def main(args):
    ...
    list_data_dict = jload('usmle_doc_open_test.json')
    ...
    for i, example in enumerate(list_data_dict):
        ...
        selected_indices = torch.argsort(scores)[-4:]
...
def main2(args):
    ...
    list_data_dict = jload('mmlu_open_test.json')
    num_doc = 5
    ...
    for i, example in enumerate(list_data_dict):
        ...
        selected_indices = torch.argsort(scores)[-6:]
        doc_select = [doc[i][j] for j in selected_indices]
        sources.append('Reference:\n' + '\nReference:\n'.join(doc_select) + '\nQuestion:\n' + list_data_dict[i]['input'] + '\nPlease only give the correct option. Correct Answer:')
