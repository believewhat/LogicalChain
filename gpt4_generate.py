...
with open("gpt4_reason.json", 'r', encoding='utf-8') as file:
    data = json.load(file)
"""
for x in data:
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": "Provide the correct answer. You should not only give me the correct option but also the reasons"}
        ]
    )
    result = response.choices[0].message.content
    x['gpt4_answer'] = result

with open(f"gpt4_reason.json", "w") as file:
    json.dump(data, file)
"""
ipdb.set_trace()
...
for x in data:
    ...
    x['gpt4_rate_q1'] = json_object["diagnosis 1"]["question 1"]
    x['gpt4_rate_q2'] = json_object["diagnosis 1"]["question 2"]
    x['gpt4_rate_q3'] = json_object["diagnosis 1"]["question 3"]
    x['gpt4_rate'] = json_object["overall score"]
...
for x in data:
    if 'gpt4_rate' in x:
        score_num += 1
        score += x['gpt4_rate']
        score_q1 += x['gpt4_rate_q1']
        score_q2 += x['gpt4_rate_q2']
        score_q3 += x['gpt4_rate_q3']
