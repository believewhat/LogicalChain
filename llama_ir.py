...
class LlamaIRForCausalLM(LlamaForCausalLM):
    ...
    def forward(self, 
        ...
        Q = self.colbert.query(*query_token[0])
        D, D_mask = self.colbert.doc(*doc_token[0], keep_dims='return_mask')

        # Repeat each query encoding for every corresponding document.
        scores = colbert_score(Q, D, D_mask, config=self.colbert_config)
        ...
        if total_loss != 0:
            ...
            print(total_loss / num, num, loss_list)
        return output
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None,
        ...
        "input_ids", "labels", "doc_token", "query_token", "doc_input_ids"
        doc_token = kwargs.pop("doc_token", None)
        query_token = kwargs.pop("query_token", None)
        doc_input_ids = kwargs.pop("doc_input_ids", None)
        inputs = super().prepare_inputs_for_generation(
            input_ids, past_key_values=past_key_values, inputs_embeds=inputs_embeds, **kwargs
        )

        inputs['doc_token'] = doc_token
        inputs['query_token'] = query_token
        inputs['doc_input_ids'] = doc_input_ids
        return inputs
