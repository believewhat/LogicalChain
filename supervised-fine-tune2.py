...
def preprocess(
    ...
    doc1 = [''.join(DEFAULT_DOC_START_TOKEN + word + DEFAULT_DOC_END_TOKEN for word in sublist) for sublist in doc]
    examples = [d + s + DEFAULT_DOC_END_TOKEN + t for d, s, t in zip(doc1, sources, targets)]
    ...
    for i in range(len(input_ids)):
        doc_start_ids, doc_end_ids = tokenizer.convert_tokens_to_ids([DEFAULT_DOC_START_TOKEN, DEFAULT_DOC_END_TOKEN])
        
        index_start = (input_ids[i] == torch.tensor(doc_start_ids)).nonzero(as_tuple=True)[0]
        index_end = (input_ids[i] == torch.tensor(doc_end_ids)).nonzero(as_tuple=True)[0]
        doc_end_ids_list.append(torch.tensor(index_end))

        temp_input_ids0 = [input_ids[i][0]]
        temp_input_ids0.extend(input_ids[i][index_end[-2]+1:index_end[-1]])
        q_len0 = len(temp_input_ids0)
        temp_input_ids0.extend(input_ids[i][index_end[-1]:])
        temp_input_ids0 = torch.tensor(temp_input_ids0)
        input_ids0.append(temp_input_ids0)

        labellist0 = copy.deepcopy(temp_input_ids0)
        labellist0[:q_len0+1] = IGNORE_INDEX
        labels0.append(labellist0)

        temp_sub_doc_input_ids = []
        temp_sub_label_input_ids = []

        for j in range(len(index_start)):
            templist = [input_ids[i][0]]
            templist.extend(input_ids[i][index_start[j]:index_end[j]+1])
            templist.extend(input_ids[i][index_end[-2]+1:index_end[-1]])
            q_len = len(templist)
            templist.extend(input_ids[i][index_end[-1]:])
            templist = torch.tensor(templist)
            temp_sub_doc_input_ids.append(templist)
            labellist = copy.deepcopy(templist)
            labellist[:q_len+1] = IGNORE_INDEX
            temp_sub_label_input_ids.append(labellist)
        sub_doc_input_ids.append(temp_sub_doc_input_ids)
        sub_label_input_ids.append(temp_sub_label_input_ids)

    

    labels = copy.deepcopy(input_ids)
    for i, label in enumerate(labels):
        label[:doc_end_ids_list[i][-1]+1] = IGNORE_INDEX
    return dict(input_ids=input_ids, labels=labels, text=sources, sub_doc_input_ids=sub_doc_input_ids, sub_label_input_ids=sub_label_input_ids, input_ids0=input_ids0, labels0=labels0)
