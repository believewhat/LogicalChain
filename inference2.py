...
def main(args):
    ...
    if not args.peft_path:
        ...
        model = LlamaIRForCausalLM.from_pretrained(config.base_model_name_or_path,
                                  config=config_base,
    ...
    list_data_dict = jload('amboss_test_doc.json')

    sources = []

    doc = [example["doc"][:30] for example in list_data_dict]
    ...
    for i, example in enumerate(list_data_dict):
        ...
        sources.append('Reference:\n' + '\nReference:\n'.join(doc_select) + '\nQuestion:\n' + list_data_dict[i]['input'] + '\nPlease not only give the correct option Correct Answer:')
    ...
    for i in range(len(results)):
        try:
            if len(outputs[i]) > 5:
                outputs[i] = re.findall(r"Correct Answer: ([A-Z])", outputs[i])[0]
            if results[i][0] == outputs[i][0]:
                acc += 1
            else:
    ...
    with open(f'results/run_7b_ft_our_usmle.txt', 'w') as file:
        file.write('\n'.join(result))
