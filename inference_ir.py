...
def main(args):
    ...
    list_data_dict = jload('usmle_open_test.json')
    ...
    for i in range(len(results)):
        if results[i][0] != outputs[i]:
            wrong.append(list_data_dict[i])
            wrong[-1]['rag'] = results[i][0]
            wrong[-1]['rag_input'] = sources[i]
    with open(f'wrong_20.json', 'w') as file:
        json.dump(wrong, file, indent=4)
    for i in range(len(wrong_answer)):
        error_20[i]['rag'] = wrong_answer[i]
    """
    correct = []
    for i in range(len(results)):
        if results[i][0] == outputs[i]:
            correct.append(list_data_dict[i])
            correct[-1]['jmlr'] = results[i][0]
            correct[-1]['jmlr_input'] = sources[i]
    ipdb.set_trace()
    cal_acc(results, outputs)
    with open(f'correct_20.json', 'w') as file:
