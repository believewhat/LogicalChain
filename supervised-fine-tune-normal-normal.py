...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        # 随机选择 2.5% 的数据来修改 output
        sample_size = max(1, int(0.09 * len(list_data_dict2)))
        sampled_indices = set(random.sample(range(len(list_data_dict2)), sample_size))  # 获取随机的索引集合

        list_data_dict = []
        
        for i, x in enumerate(list_data_dict2):
            
            # 对每个数据构建输入，包含前3个doc
            truncated_docs = x['doc'][:3]
            new_example = x.copy()
            
            """
            # 如果这个数据的索引在2.5%的范围内，修改它的 output
            
            if i in sampled_indices:
                new_example['input'] = 'Reference:\n' + '\nReference:\n'.join(truncated_docs) + '\nQuestion:\n' + x['input'].replace('Correct Answer:', '') + '. Correct Answer:\nBioReference:'
                new_example['output'] = '\nReference:\n'.join(truncated_docs)
            else:
                new_example['input'] = 'Reference:\n' + '\nReference:\n'.join(truncated_docs) + '\nQuestion:\n' + x['input'].replace('Correct Answer:', '') + '. Correct Answer:'
                new_example['output'] = x['output']
            list_data_dict.append(new_example)
            """
        ...
        # 处理输入和目标输出
        self.sources = [example["input"] for example in list_data_dict]
