...
def main(args):
    ...
    file_name = 'zero_shot_mmlu_test.json'
    if 'mmlu' not in file_name:
        list_data_dict = jload(file_name)[:100]
    else:
        list_data_dict = []
        ...
        with open('zero_shot_mmlu_val.json', 'r', encoding='utf-8') as file:
            for line in file:
                try:
                    json_obj = json.loads(line)
                    if json_obj['subject'] not in med_tasks:
                        continue
                    list_data_dict.append(json_obj)
                except:
                    continue
        #list_data_dict = jload('zero_shot_mmlu_val.json')

    num_doc = 5

    nbits = 2
    checkpoint_path = args.colbert_model
    experiment_root = "./experiments/"
    experiment_name = "openguideline_medmcqa"
    index_name = f'{experiment_name}.{nbits}bits'
    collection = './open_guideline/open_medguideline_qa.tsv'
    ...
    for i, example in enumerate(list_data_dict):
        ...
        selected_indices = torch.argsort(scores)[-3:]
    ...
    outputs = []
    for x in sources:
        outputs.append(re.sub(r'\nQuestion:\n.*', '', x, flags=re.DOTALL))
    from rouge_score import rouge_scorer
    scorer = rouge_scorer.RougeScorer(['rouge1', 'rouge2', 'rougeL', 'rougeLsum'], use_stemmer=True)

    # 计算每对摘要之间的分数
    scores = []
    for system, reference in zip(results, outputs):
    ...
    ipdb.set_trace()
    with open('trigger_llama_medqa.json', 'w') as file:
        json.dump(results, file, indent=4)
    import matplotlib.pyplot as plt
    plt.figure()
    plt.hist(rougeLsum, bins=10, alpha=0.75, color='blue', edgecolor='black')
    plt.savefig('trigger_llama.jpg')

    exact_acc = 0
    for system, reference in zip(results, outputs):
        references = system.split('Reference:')
        for x in references:
            if len(x) <= 100:
                print(x)
                continue
            if x in reference or system in x:
                exact_acc += 1
                break
